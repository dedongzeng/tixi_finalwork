...
TensorFlow 2.0 implementation of a Recurrent Neural Network (LSTM)
that performs dynamic computation over sequences with variable length.
This example is using a toy dataset to classify linear sequences.
...
seq_max_len = 1200 # Maximum sequence length.
seq_min_len = 500 # Minimum sequence length (before padding).
...
print('train_data shape: {}'.format(train_data))

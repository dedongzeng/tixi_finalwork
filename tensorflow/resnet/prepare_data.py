...
def randgen_dataset(num_batches, img_h=32, img_w=32):
    # original cifar-10 dataset:
    # train_x shape: (50000, 3, 32, 32)
    # train_y shape: (50000, 1)
    train_x = np.random.randn(num_batches, 3, img_h, img_w).astype(np.float32)
    train_y = np.random.randn(num_batches, 1).astype(np.int32)
    dataset = tf.data.Dataset.from_tensor_slices((train_x, train_y))
    return dataset, len(train_x)

def generate_datasets():

    train_dataset, train_count = randgen_dataset(1024*32, 32, 32)
    valid_dataset, valid_count = randgen_dataset(1024, 32, 32)
    test_dataset, test_count = randgen_dataset(1024, 32, 32)
